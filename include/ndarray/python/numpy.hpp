#ifndef NDARRAY_PYTHON_numpy_hpp_INCLUDED
#define NDARRAY_PYTHON_numpy_hpp_INCLUDED

/** 
 *  @file ndarray/python/numpy.hpp
...
struct PyConverter< Array<T,N,C> > : public detail::PyConverterBase< Array<T,N,C> > {
    ...
    static bool fromPythonStage2(
        ...
        NDARRAY_ASSERT(PyArray_Check(input.get()));
        NDARRAY_ASSERT(reinterpret_cast<PyArrayObject*>(input.get())->nd == N);
        NDARRAY_ASSERT(reinterpret_cast<PyArrayObject*>(input.get())->flags & flags);
        PyPtr array(input);
...
#endif // !NDARRAY_PYTHON_numpy_hpp_INCLUDED
